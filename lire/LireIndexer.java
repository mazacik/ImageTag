...
public class LireIndexer {
	public LireIndexer(String directory) {
		...
		for (String imageFilePath : images) {
			InstanceManager.getLogger().debug(images.indexOf(imageFilePath) + "/" + images.size() + "\tIndexing " + imageFilePath);
			try {
				BufferedImage img = ImageIO.read(new FileInputStream(imageFilePath));
				Document document = globalDocumentBuilder.createDocument(img, imageFilePath);
				iw.addDocument(document);
			} catch (Exception e) {
				InstanceManager.getLogger().error("Error reading the image or indexing it.");
				e.printStackTrace();
			}
		}
		try {
