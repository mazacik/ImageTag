...
public abstract class ThumbnailCreator {
	...
	private static Image createThumbnailFromImage(DataObject dataObject, int thumbSize, File cacheFile) {
		try {
			...
			if (writableImage != null) ImageIO.write(writableImage, "jpg", cacheFile);
			else
				InstanceManager.getLogger().error("FXUtils was unable to create a cache image for " + dataObject.getPath());
			return thumbnail;
		} catch (IOException e) {
			e.printStackTrace();
			return null;
